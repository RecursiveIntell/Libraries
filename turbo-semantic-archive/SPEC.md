...
**Performance Bounds (Brute-Force Vector Search)**
...
1. **Namespace partitioning:** Split large namespaces into sub-namespaces.
2. **ANN index:** Add an HNSW or IVF index (e.g., the `usearch` crate).
3. **Pre-filtering:** Add metadata columns and filter in SQL before loading BLOBs.
4. **Quantization:** Store int8 quantized embeddings and use dot-product approximation.

For the target use case (single-user agent, <50K facts/chunks), brute-force is the right choice.
