...
/// A legacy superstep checkpoint value retained for API compatibility.
///
/// New governed durable execution uses [`crate::checkpoint_store::CheckpointStore`]
/// and [`crate::checkpoint_store::SqliteCheckpointStore`]. This type remains
/// available for existing graph consumers and is not used by the daemon owner.
...
/// Legacy SQLite checkpoint manager retained for compatibility.
///
/// New governed durable execution must use `SqliteCheckpointStore`. This
/// explicit compatibility facade does not participate in daemon-mode storage.
pub struct CheckpointManager {
    conn: Connection,
}

impl CheckpointManager {
    /// Create a new legacy checkpoint manager.
    ...
    /// Load the most recent legacy checkpoint for an execution.
    ...
    /// Load all legacy checkpoints for an execution, ordered by timestamp.
    ...
    /// Delete all legacy checkpoints for an execution.
