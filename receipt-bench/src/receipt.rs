//! Benchmark receipt — timestamped provenance record for a benchmark run.
...
impl BenchmarkReceipt {
    ...
    pub fn receipt_hash(&self) -> String {
        ...
        hasher.update(self.machine_fingerprint.as_str().as_bytes());
    ...
    pub fn to_json(&self) -> Result<Vec<u8>, crate::error::BenchError> {
        serde_json::to_vec(self).map_err(|e| crate::error::BenchError::Serialization(e.to_string()))
    }

    /// Deserialize a receipt from JSON bytes.
    pub fn from_json(bytes: &[u8]) -> Result<Self, crate::error::BenchError> {
...
mod tests {
    ...
    fn dummy_fingerprint() -> MachineFingerprint {
        // Use a deterministic fingerprint for testing
        MachineFingerprint::from_hex(&"0".repeat(64))
    }
    ...
    fn test_receipt_serialization() {
        ...
        let deserialized = BenchmarkReceipt::from_json(&json).unwrap();
