...
impl SemanticMemoryServer {
    pub fn new(
        ...
        let full_only_tools: &[&str] = &["sm_create_session", "sm_add_message", "sm_list_sessions"];
    ...
    fn llm_generate_json(&self, body: &serde_json::Value) -> Option<serde_json::Value> {
        if self.llm_url.trim().is_empty() || self.llm_model.trim().is_empty() {
            return None;
        }
        let generate_url = format!("{}/api/generate", self.llm_url.trim_end_matches('/'));
        ...
        match result {
            Ok(id) => {
                ...
                if extract_entities == Some(true) {
                    ...
                    if let Some(v) = self.llm_generate_json(&body) {
            ...
            .map(|c| {
                let summary: Option<String> = if summarize && !c.members.is_empty() {
                    ...
                    if !member_texts.is_empty() {
                        ...
                        self.llm_generate_json(&body).and_then(|v| {
                            v.get("response")
                                .and_then(|r| r.as_str())
