...
This crate is currently a member of the `Libraries` Rust workspace and uses local path dependencies for `llm-output-parser`, `llm-tool-runtime`, and `stack-ids`.
...
For an external consumer, use the package metadata and published source that correspond to the version you have admitted. Do not assume the repository’s path dependencies can be copied unchanged into an unrelated workspace.
