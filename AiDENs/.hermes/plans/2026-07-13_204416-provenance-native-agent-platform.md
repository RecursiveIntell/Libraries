...
- Fresh Batch C source inspection (2026-07-14) found `llm-tool-runtime` now owns the invocation-level authority/effect seam: non-`Clone` one-shot `ToolExecutionPermit` values with atomic in-process consumption, expiry, method/effect-digest binding; typed `EffectIntent` alias/compound-target normalization; durable preflight/outcome receipt handling; an unresolved-outcome marker; and receipt authority-lineage fields. The targeted authority hardening integration test is green (8/8), but this is **not yet a dependable foundation**: the current package all-target test target fails to compile on three existing `ArtifactId::from(&str)` call sites, and the package library suite has two current failures due to changed canonical ID rendering (`remote-oracle-lease:lease-1` versus `lease-1`, and doubled `artifact:` prefix). The claimed 74-test package pass is therefore not reproduced in the live combined tree.
- Before the gateway consumes this seam, remediate/verify: durable cross-restart permit spending (not only `AtomicBool` process-local protection); atomic reconciliation of permit consumption and durable preflight; canonical deterministic IDs instead of UUIDs in permit issuance; no authority widening through aliases/compound targets; complete causal preflight/outcome linkage; outcome uncertainty recovery; protected/redacted secret-bearing arguments; and fail-closed parity in debug and release paths. The gateway must adapt these contracts rather than replicate them.
...
- Fresh Libraries-root Batch H evidence (2026-07-14) SHA-pinned all eight `uses:` action references, added action-pinning/supply-chain/workspace-hygiene checks, and passed generated release-truth drift checks. Fresh `cargo metadata` observed 64 declared workspace members, 59 defaults, and 64 unique packages. These are generated observations—not a release or source-of-truth certification.
- The same fresh evidence proves the root supply-chain gate fails closed: `cargo deny check` exits 3 on two wildcard path dependencies in `hnsw-bench`, unmaintained `bincode` through `hnsw_rs`, and vulnerable `memmap2 0.9.10` (upgrade required to >=0.9.11). These blockers are outside Batch H's permitted edits and must not be opportunistically changed during gateway work.
