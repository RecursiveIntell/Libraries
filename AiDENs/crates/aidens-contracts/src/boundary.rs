...
impl BoundaryRepairReceiptV1 {
    pub fn new(
        ...
        let material = format!(
            "{}|{}|{}",
            profile.profile_id.as_str(), repair_kind, treatment.receipt_id.as_str()
        );
        Self {
...
impl BoundaryCompileReceiptV1 {
    pub fn accepted(
        ...
        let material = format!(
            "{}|{}|{}",
            profile.profile_id.as_str(), input_digest.digest, output_digest.digest
        );
        Self {
    ...
    pub fn rejected_duplicate_key(
        ...
        let material = format!(
            "{}|{}|duplicate-key",
            profile.profile_id.as_str(), input_digest.digest
        );
        Self {
            receipt_id: generated_artifact_id_from_material("boundary-compile", &material),
            profile_id: profile.profile_id.clone(),
