...
Prove one operator-to-receipt-to-forge-to-bridge-to-memory-to-runtime-to-CLI flow.
...
Edit runner/CLI/adapters/testkit.

## Forbidden changes

Must not fake the vertical slice with only local mocks.

Always forbidden: using `~/Coding/Libraries2/stack-ids`; adding local canonical truth types; starting blocked later-phase work.

## Tasks

Implement the golden vertical slice using actual stack crates. No daemon/kernel expansion.
...
```bash
./scripts/run_codex_phases.sh verify 03
```
...
```text
PHASE COMPLETED: 03 — Golden vertical slice
FILES CHANGED:
TESTS ADDED/UPDATED:
GATES RUN:
GATES PASSED:
