...
Expose kernel/oracle as adapter over canonical compiler/execution/oracle/conformance crates.
...
Edit kernel adapter/testkit only after prior gates.

## Forbidden changes

Must not define local kernel receipts or convergence truth.

Always forbidden: using `~/Coding/Libraries2/stack-ids`; adding local canonical truth types; starting blocked later-phase work.

## Tasks

Only after spine passes, expose canonical kernel/oracle via adapter. No local kernel receipt truth.
...
- `kernel_exact_small_slice`
- `loopy_nonconvergence_degrades`

## Acceptance gates

```bash
./scripts/run_codex_phases.sh verify 08
```

If cargo is available, run phase tests directly, for example:

```bash
cargo test -p aidens-testkit kernel_exact_small_slice
```

## Final report format

```text
PHASE COMPLETED: 08 — Kernel/oracle integration
FILES CHANGED:
TESTS ADDED/UPDATED:
GATES RUN:
GATES PASSED:
