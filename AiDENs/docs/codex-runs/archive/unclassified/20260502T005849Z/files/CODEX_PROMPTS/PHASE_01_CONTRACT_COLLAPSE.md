...
Reduce aidens-contracts to app-only/re-export and eliminate P0 duplicate canonical types.
...
Edit aidens-contracts, manifests, tests.

## Forbidden changes

Must not preserve local truth types as public canonical APIs.

Always forbidden: using `~/Coding/Libraries2/stack-ids`; adding local canonical truth types; starting blocked later-phase work.

## Tasks

Collapse `aidens-contracts`: make canonical stack concepts aliases/re-exports over library types. Do not add compatibility DTOs.
...
- `contract_owner_proof`
- `canonical_id_roundtrip`

## Acceptance gates

```bash
./scripts/run_codex_phases.sh verify 01
```

If cargo is available, run phase tests directly, for example:

```bash
cargo test -p aidens-testkit contract_owner_proof
```

## Final report format

```text
PHASE COMPLETED: 01 — Contract collapse
FILES CHANGED:
TESTS ADDED/UPDATED:
GATES RUN:
GATES PASSED:
