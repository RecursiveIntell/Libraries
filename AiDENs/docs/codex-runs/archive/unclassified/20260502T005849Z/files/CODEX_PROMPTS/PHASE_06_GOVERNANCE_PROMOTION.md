...
Enforce verification plans, approval, promotion/refutation/rollback via canonical verification crates.
...
Edit governance/delegation/permit/runner/testkit.

## Forbidden changes

Must not allow model-only promotion.

Always forbidden: using `~/Coding/Libraries2/stack-ids`; adding local canonical truth types; starting blocked later-phase work.

## Tasks

Governance and promotion: verification/control/policy/adjudication paths deny promotion without verification and require side-effect approval.
...
- `promotion_denies_without_verification`
- `approval_required_for_side_effect`
- `repair_record_backpointer`

## Acceptance gates

```bash
./scripts/run_codex_phases.sh verify 06
```

If cargo is available, run phase tests directly, for example:

```bash
cargo test -p aidens-testkit promotion_denies_without_verification
```

## Final report format

```text
PHASE COMPLETED: 06 — Governance/promotion
FILES CHANGED:
TESTS ADDED/UPDATED:
GATES RUN:
GATES PASSED:
