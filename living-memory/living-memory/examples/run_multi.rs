...
const STRATEGIES: &[Strategy] = &[
    ...
    Strategy { name: "insert-mod",   file: "src/lib.rs", op_kind: "insert",  target_line: 1, context: "//! Module-level doc comment" },
    Strategy { name: "delete-mod",   file: "src/lib.rs", op_kind: "delete",  target_line: 1, context: "//! Module-level doc comment" },
    Strategy { name: "replace-mod",  file: "src/lib.rs", op_kind: "replace", target_line: 1, context: "//! Module-level doc comment" },
    // Different file paths (only for cea-core which has submodules)
    Strategy { name: "insert-attrib", file: "src/attribution.rs", op_kind: "insert", target_line: 1, context: "fn attribute_effects" },
    Strategy { name: "insert-types",  file: "src/types.rs",      op_kind: "insert", target_line: 1, context: "pub struct EditOpSignature" },
    Strategy { name: "insert-graph",  file: "src/graph.rs",      op_kind: "insert", target_line: 1, context: "pub struct CausalGraph" },
];
...
fn build_patch(s: &Strategy) -> StructuredPatch {
    ...
    let ops = match s.op_kind {
        "delete" => vec![EditOp::Delete {
            range: LineRange { start: s.target_line, end_exclusive: s.target_line + 1 },
        }],
...
async fn main() {
    ...
    println!("Suite: {} ({} tasks, {} strategies)\n", suite.name, suite.tasks.len(), STRATEGIES.len());
    ...
    for task in &suite.tasks {
        eprintln!("DEBUG: task={} path={}", task.task.task_id, task.fixture_path.display());
        for s in STRATEGIES {
            ...
            match runner.run(&task.fixture_path, &patch, &experiment_config).await {
                Ok(experiment) => {
                    ...
                    if !attributed.is_empty() {
                        let scope = attributed.first().map(|t| format!("{:?}", t.cause.scope_tag)).unwrap_or_default();
    ...
    println!("\n=== DONE ===");
    println!("edges: {}  no-triples: {}  dups: {}", total_edges, total_no_triples, total_dups);
}
