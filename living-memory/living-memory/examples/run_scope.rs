...
use forge_engine::runtime::patch::types::{
    Anchor, EditOp, FileEdit, FileMode, StructuredPatch,
};
...
const TARGETS: &[ScopeTarget] = &[
    ...
    ScopeTarget {
        name: "fn-attrib", file: "src/attribution.rs", target_line: 16,
        context: &["pub struct AttributionTriple {"],
        op_kind: "insert",
    },
    ScopeTarget {
        name: "fn-predict", file: "src/predict.rs", target_line: 32,
        context: &["pub fn predict("],
        op_kind: "insert",
    },
    // struct scope
    ScopeTarget {
        name: "struct-graph", file: "src/graph.rs", target_line: 18,
        context: &["pub struct EdgeStats {"],
        op_kind: "insert",
    },
    // enum scope
    ScopeTarget {
        name: "enum-error", file: "src/error.rs", target_line: 2,
        context: &["pub enum CeaCoreError {"],
        op_kind: "insert",
    },
    // impl scope
    ScopeTarget {
        name: "impl-stats", file: "src/graph.rs", target_line: 37,
        context: &["impl EdgeStats {"],
        op_kind: "replace",
    },
    // trait scope (via mod in lib.rs with context)
    ScopeTarget {
        name: "mod-lib", file: "src/lib.rs", target_line: 12,
        context: &["mod attribution;"],
        op_kind: "insert",
    },
    // Another fn scope with replace
    ScopeTarget {
        name: "fn-cal-replace", file: "src/calibration.rs", target_line: 53,
        context: &["pub fn advisory_confidence("],
        op_kind: "replace",
    },
    // macro_rules scope
    ScopeTarget {
        name: "macro-delete", file: "src/types.rs", target_line: 1,
        context: &[],
        op_kind: "delete",
    },
];
...
async fn main() {
    ...
    println!("Fixture: {}  Tasks: {}  Targets: {}\n", fixture_dir.display(), suite.tasks.len(), TARGETS.len());
