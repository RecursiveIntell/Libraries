...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT embedding FROM facts WHERE embedding IS NOT NULL ORDER BY RANDOM() LIMIT 10",
    )?;
    ...
    for (i, query) in queries.iter().enumerate() {
        ...
        let overlap = pd.hits.iter().filter(|h| ids.contains(&h.id)).count();
