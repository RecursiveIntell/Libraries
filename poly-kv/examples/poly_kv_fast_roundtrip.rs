//! `poly_kv_fast_roundtrip` — fast compress+decompress for the PPL validation.
//!
//! Same input format as `poly_kv_dynamic_cache_roundtrip`. Output is a single
...
//!
//! Usage: poly_kv_fast_roundtrip <input.json> <output.bin>
...
use poly_kv::codec::FibQuantAdapter;
...
fn main() {
    ...
    let layer_data: Vec<Vec<u8>> = (0..num_layers)
        ...
        .map(|layer_idx| {
            ...
            // We need to transpose to per-head layout for the poly-kv
