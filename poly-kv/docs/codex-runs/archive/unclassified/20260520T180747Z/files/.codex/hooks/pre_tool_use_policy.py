...
for pat in deny_patterns:
    if re.search(pat, text, re.I):
        print(f"Blocked risky command pattern: {pat}", file=sys.stderr)
        write_receipt("pre_tool_use_policy", payload, status="blocked")
        sys.exit(2)
write_receipt("pre_tool_use_policy", payload)
sys.exit(0)
