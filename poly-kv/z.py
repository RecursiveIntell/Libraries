...
def check_context_package_evidence(root: Path, archive_root: Path, included: Sequence[Path], policy: Policy) -> list[Finding]:
    findings: list[Finding] = []
    ...
    command_evidence = [
        rel for rel in included_root_rels
        if rel.startswith(".codex-runs/")
        and (rel.endswith("/commands_run.log") or rel.endswith("/commands_run.receipts.jsonl"))
    ]
    if not command_evidence:
        findings.append(Finding(
            ...
            severity="error",
            path=".codex-runs/",
            detail="Context/audit package manifest must include commands_run.log or commands_run.receipts.jsonl evidence.",
        ))
...
def build(args: argparse.Namespace) -> BuildResult:
    ...
    findings.extend(check_context_package_evidence(root, archive_root, included, policy))
