#!/usr/bin/env python3
"""
ppl_validate.py — Real-LLM PPL validation for poly-kv.
...
- Phase 1: Compressed roundtrip. Convert oracle cache to poly-kv corpus JSON,
  invoke the poly_kv_fast_roundtrip CLI, read the decompressed layers,
...
def phase1_compressed(args, state: dict) -> None:
    """
    Read cache_oracle.pt, convert to poly-kv corpus JSON, invoke the
    poly_kv_fast_roundtrip CLI to compress+decompress, read
    ...
    # 1) Convert DynamicCache to poly-kv corpus JSON
    print("[phase1] building poly-kv corpus from oracle cache", flush=True)
    corpus_path = args.output.parent / "poly_kv_corpus.json"
    cmd_build = [
        sys.executable, str(args.script_dir / "build_poly_kv_corpus.py"),
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"poly_kv CLI failed with code {proc.returncode}")
...
def phase2_report(args, state: dict) -> None:
    ...
    lines = [
        ...
        f"   - Invoke `poly_kv_fast_roundtrip` CLI (composite build + parallel decompress) "
    ...
    lines += [
        ...
        f"- `poly_kv_corpus.json` — Phase 1 input to the poly-kv CLI",
        f"- `roundtrip.bin` — Phase 1 binary output (manifest + 24 layer blobs)",
        f"- `manifest` (in `roundtrip.bin`) — pool manifest from poly-kv",
        "",
        "## Caveats",
        "",
...
def main() -> int:
    ...
    p.add_argument(
        "--poly-kv-cli", type=Path,
        default=Path(
            "/home/jstevenson/Coding/Libraries/poly-kv/target/release/examples/"
            "poly_kv_fast_roundtrip"
        ),
        help="Path to the poly_kv_fast_roundtrip binary",
    )
    p.add_argument(
        "--script-dir", type=Path,
        default=Path(__file__).resolve().parent,
        help="Directory containing the helper scripts (build_poly_kv_corpus.py)",
    )
