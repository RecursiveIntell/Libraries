...
- `distilgpt2_attention_speed_bench.py` isolates exact dense attention vs compressed sparse candidate attention over precomputed DistilGPT2 Q/K/V tensors. The benchmark now excludes setup and quality diagnostics from the optimized/prequantized timed hot path. Current receipt remains a negative speed result: 20.59x decode-work reduction, scalar speed ratio 0.1619x, vectorized-with-setup speed ratio 0.2028x, and optimized/prequantized speed ratio 0.4251x on NumPy CPU. It proves decode-work reduction and removes benchmark overhead bugs, but still does not prove runtime speedup.
