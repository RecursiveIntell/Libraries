...
- `distilgpt2_attention_speed_bench.py` isolates exact dense attention vs compressed sparse candidate attention over precomputed DistilGPT2 Q/K/V tensors. The benchmark now includes five timed paths: scalar, vectorized-with-setup, optimized/prequantized, batch-optimized (all-positions matmul), and Quest-style page-filtered (arXiv:2406.10774). Current receipt: 20.59x decode-work reduction, batch speed ratio 0.5822x, optimized 0.4585x, quest 0.1752x, scalar 0.1502x, vectorized 0.1977x. Batch-optimized is the fastest compressed path with some cases exceeding 1.0x. It proves decode-work reduction and identifies batch position processing as the highest-ROI Python-level optimization, but still does not prove aggregate runtime speedup.
- Rust `PreparedCompressedIndex` and `FullyPreparedCompressedIndex` APIs cache decoded key/value codes, pre-unpacked indices, pre-decoded norms, and built FibScorer, avoiding per-call codec reconstruction and per-call bit-unpacking. Fair isolated Rust benchmark (release mode, scale sweep 64-2048 tokens, head_dim=8 and 64) shows fully prepared compressed attention BEATS pre-decoded exact dense at 1024 tokens head_dim=8 (1.70x) and at 2048 tokens head_dim=64 (1.44x). This is the first positive speed result in a fair operator-to-operator comparison. The break-even point is around 512-1024 tokens depending on head_dim. Top-k overlap is 0.23-0.78 on synthetic random vectors (expected for unstructured data; real attention patterns should be higher).
