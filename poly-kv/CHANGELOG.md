...
- Rust `PreparedCompressedIndex` API caches decoded key/value codes and built FibScorer, avoiding per-call codec reconstruction. Fair isolated Rust benchmark (pre-decoded exact dense vs prepared compressed, release mode, scale sweep 64-512 tokens) shows prepared compressed is 2.9-5.2x slower than pre-decoded exact dense matmul, but 3000-8000x faster than regular compressed (which rebuilds codec per call). The speed ratio improves with token count (0.19x at 64, 0.35x at 512), suggesting prepared compressed will break even at larger scales. Top-k overlap is 0.45-0.78 on synthetic random vectors. This is a fair operator-to-operator benchmark, not a decompress-inflated win.
