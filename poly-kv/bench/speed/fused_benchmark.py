"""Benchmark fused CUDA kernels vs Python vs dense matmul."""
...
from fused_scorer import PerDimScorerFused, PerKeyScorerFused


def time_fn(fn, n_warmup=10, n_runs=100):
...
def main():
    ...
    for n_keys in n_keys_list:
        ...
        # Dense baseline
        t_dense = time_fn(lambda: torch.mv(keys, query))
        ...
        t_pd_fused = time_fn(lambda: scorer_pd.score(codes, k_norms, scaled_query, bias))
        ...
        t_pd_py = time_fn(score_pd_python)
        ...
        t_pk_fused = time_fn(lambda: scorer_pk.score(qk, scales, query))
        ...
        t_pk_py = time_fn(score_pk_python)
    ...
    # Save results
    output_file = Path(__file__).parent / 'fused_benchmark_results.json'
