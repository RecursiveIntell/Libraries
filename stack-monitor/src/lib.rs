...
pub use projections::{
    CoverageOwnerProjection, CoverageProjection, HealthProjection, ProjectionService,
    TimelineProjection,
};
#[cfg(feature = "semantic-memory-bridge")]
...
pub(crate) mod test_support {
    ...
    static GLOBAL_SINK_TEST_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

    pub(crate) fn global_sink_guard() -> MutexGuard<'static, ()> {
        match GLOBAL_SINK_TEST_LOCK.get_or_init(|| Mutex::new(())).lock() {
            Ok(guard) => guard,
