...
fn profile_p1_fixture_bundles_parse_into_owned_types() {
    for name in [
        ...
        if let Some(value) = artifacts.get("PrivacyRetentionProfileV1") {
            let _: PrivacyRetentionProfileV1 =
                serde_json::from_value(value.clone()).expect("PrivacyRetentionProfileV1");
        }
        if let Some(value) = artifacts.get("RedactionRuleSetV1") {
            let _: RedactionRuleSetV1 =
                serde_json::from_value(value.clone()).expect("RedactionRuleSetV1");
        }
        if let Some(value) = artifacts.get("AccessPurposeMatrixV1") {
            let _: AccessPurposeMatrixV1 =
                serde_json::from_value(value.clone()).expect("AccessPurposeMatrixV1");
        }
        if let Some(value) = artifacts.get("AuditExtractionPolicyV1") {
            let _: AuditExtractionPolicyV1 =
                serde_json::from_value(value.clone()).expect("AuditExtractionPolicyV1");
        }
    }
}
