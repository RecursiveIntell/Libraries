# Libraries Reference (v2)

Complete architectural reference for every crate and package in this repository. Describes current state, not aspirational future state.
...
4. [LLM-Pipeline](#llm-pipeline) - LLM execution payloads: prompts, backends, parsing, retry
...
**Status:** Compiles with 5 dead-code warnings (internal functions)
...
20 test files covering: chunking, CRUD, search scoring, migrations, integrity, concurrency, HNSW integration/persistence/hotswap, quantization, compaction.
...
**Status:** Compiles clean (0 warnings), 51 unit tests passing
...
```
knowledge-runtime/
├── lib.rs          -- Crate docs (honest: "Implemented Now" / "Not Real Yet"), re-exports
├── runtime.rs      -- KnowledgeRuntime: query pipeline, projection maintenance API surface
...
├── projection/
│   └── lifecycle.rs -- ProjectionTracker with StaleCause, version metadata, invalidation APIs
├── temporal/
│   └── claims.rs   -- TemporalClaim, ContradictionStatus (structural overlap only)
├── evidence/
...
| Result merge with provenance fusion | Duplicates fused (not discarded), union of source legs, per-leg scores, configurable boost |
| Scope-aware code identity | Code entity IDs include repo_id/workspace_id, preventing cross-repo collisions |
| Projection lifecycle tracking | Health, staleness (time + explicit), StaleCause, version metadata, invalidation by scope/kind |
...
Every query, entity, projection, and code ID is scoped by: `namespace` (required) + `domain` + `workspace_id` + `repo_id` (all optional). Only `namespace` is enforced at the search adapter level. The other dimensions are enforced in runtime-owned data structures and surfaced in traces.
...
**Status:** Compiles clean, 133 tests across 13 test files
...
├── graph.rs          -- AgentGraph, AgentGraphBuilder (65KB, core orchestrator)
...
├── checkpoint.rs     -- Legacy checkpoint structures
...
**Status:** Compiles clean (0 warnings)
...
**Status:** Standalone library, no warnings

Production-grade parser for extracting structured data from LLM responses. Handles think blocks, markdown fences, malformed JSON, and real-world model output without an additional LLM call. Used by both `LLM-Pipeline` and `Ollama-Vision-RS`.
...
**Status:** Compiles clean, 72 total tests (24 unit + 48 integration)
...
Robust Ollama vision model toolkit for image tagging and captioning. Core value is not just calling Ollama, but reliably recovering structured tags and captions from inconsistent model output using multi-strategy parsing.
...
**Status:** Compiles clean, 40+ integration tests
...
A workspace of 10 complementary Rust crates forming a unified system for patch validation, execution, causal attribution, and intelligent retry policies. These are the low-level building blocks used by `living-memory`.
...
├── runtime/
│   ├── compiler.rs  -- MindState compilation from request + repo context + evidence
...
│   └── patch/       -- validate → apply → render_diff (delegates to typed-patch)
├── exec/
...
│   ├── archive.rs   -- MAP-Elites archive (novelty × stability × approach family)
...
| Directory | Status |
|---|---|
| `llm-pipeline/` (lowercase) | Orphaned artifact from deleted duplicate crate. Only contains stale `Cargo.lock`. Not in git index. The real crate is `LLM-Pipeline/` (uppercase). |
