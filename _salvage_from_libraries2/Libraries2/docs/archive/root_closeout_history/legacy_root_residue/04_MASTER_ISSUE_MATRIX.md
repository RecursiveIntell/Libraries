...
The repo has separate fixture corpora for effect execution, delegated authority, and release assurance, but no single operator-readable path stitches them into one story that an external reviewer can run and understand in one sitting.

**Required change**

Create one demo bundle, one walkthrough README, and one validating test or script that binds effect_happy_path, delegated_effect_happy_path, and release_happy_path into a single evidentiary narrative. Continuity v24 may be added as an optional extension rather than a blocker.

**Acceptance**

- A reviewer can run one command or read one README and see preflight -> delegation -> release readiness without hand-assembling wave fixtures.
- The demo outputs typed artifacts, not prose-only claims.
- The demo README names which existing fixture bundles and owner crates participate.

**Proof**

- `docs/demos/effect_authority_assurance_release.md`
- `contracts/fixtures/demo/effect_authority_assurance_release.bundle.json`
- `verification-control/tests/e2e_effect_authority_assurance_release.rs or equivalent script`
...
The architecture is now internally consistent, but there is still no external score sheet that proves superiority on replayable evidence-bound reasoning, temporal correctness, contradiction handling, and verification yield.

**Required change**

Define the benchmark suite, publish fixture inputs and expected outputs, and add one reproducible score sheet. The benchmark must target what ordinary RAG or generic agent stacks cannot do: replayability, temporal correctness, widening disclosure, and repair honesty.

**Acceptance**

- 11_BENCHMARK_PLAN.md is implemented as runnable benchmark assets, not just aspirations.
- A score sheet compares the stack against at least one weaker baseline on temporal correctness and replayability.
- Benchmark artifacts are stored under contracts/fixtures/bench or docs/benchmarks with a reproducibility note.

**Proof**

- `bench harness or script`
- `score sheet`
- `fixture manifest`
