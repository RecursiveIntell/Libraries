...
fn profile_p2_fixture_bundles_parse_into_owned_types() {
    for name in [
        ...
        if let Some(value) = artifacts.get("ResidencyPolicyProfileV1") {
            let _: ResidencyPolicyProfileV1 = serde_json::from_value(value.clone()).expect("ResidencyPolicyProfileV1");
        }
        if let Some(value) = artifacts.get("TenantBoundaryProfileV1") {
            let _: TenantBoundaryProfileV1 = serde_json::from_value(value.clone()).expect("TenantBoundaryProfileV1");
        }
        if let Some(value) = artifacts.get("CrossBoundaryTransferClassV1") {
            let _: CrossBoundaryTransferClassV1 = serde_json::from_value(value.clone()).expect("CrossBoundaryTransferClassV1");
        }
        if let Some(value) = artifacts.get("LocalityExceptionV1") {
            let _: LocalityExceptionV1 = serde_json::from_value(value.clone()).expect("LocalityExceptionV1");
        }
    }
}
