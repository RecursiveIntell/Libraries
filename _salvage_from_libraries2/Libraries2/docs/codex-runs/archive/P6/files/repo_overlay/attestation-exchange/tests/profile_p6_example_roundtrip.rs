...
fn vendor_certification_adapter_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/vendor-certification-adapter-v1.example.json").expect("read example");
...
fn vendor_evidence_translation_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/vendor-evidence-translation-v1.example.json").expect("read example");
...
fn vendor_trust_root_binding_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/vendor-trust-root-binding-v1.example.json").expect("read example");
...
fn vendor_revocation_handling_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/vendor-revocation-handling-v1.example.json").expect("read example");
