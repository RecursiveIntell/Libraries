...
fn profile_p3_fixture_bundles_parse_into_owned_types() {
    for name in [
        "conflict_recusal_blocked",
        "delegation_matrix_happy_path",
    ] {
        ...
        if let Some(value) = artifacts.get("DelegationMatrixV1") {
            let _: DelegationMatrixV1 = serde_json::from_value(value.clone()).expect("DelegationMatrixV1");
        }
        if let Some(value) = artifacts.get("ApprovalMatrixV1") {
            let _: ApprovalMatrixV1 = serde_json::from_value(value.clone()).expect("ApprovalMatrixV1");
        }
        if let Some(value) = artifacts.get("ConflictClassCatalogV1") {
            let _: ConflictClassCatalogV1 = serde_json::from_value(value.clone()).expect("ConflictClassCatalogV1");
        }
    }
}
