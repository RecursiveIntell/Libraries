use authority_delegation::{RoleCatalogV1, DelegationMatrixV1, ApprovalMatrixV1, ConflictClassCatalogV1};

#[test]
fn role_catalog_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/role-catalog-v1.example.json").expect("read example");
...
fn delegation_matrix_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/delegation-matrix-v1.example.json").expect("read example");
...
fn approval_matrix_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/approval-matrix-v1.example.json").expect("read example");
...
fn conflict_class_catalog_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/conflict-class-catalog-v1.example.json").expect("read example");
