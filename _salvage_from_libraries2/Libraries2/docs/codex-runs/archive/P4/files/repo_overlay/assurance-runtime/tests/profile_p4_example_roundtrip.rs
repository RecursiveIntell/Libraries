...
fn regulatory_regime_profile_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/regulatory-regime-profile-v1.example.json").expect("read example");
...
fn requirement_control_map_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/requirement-control-map-v1.example.json").expect("read example");
...
fn evidence_collection_plan_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/evidence-collection-plan-v1.example.json").expect("read example");
...
fn recertification_schedule_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/recertification-schedule-v1.example.json").expect("read example");
