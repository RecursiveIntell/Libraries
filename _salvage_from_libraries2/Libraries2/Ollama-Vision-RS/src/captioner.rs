...
pub async fn caption_image(
    ...
    let prompt = options.prompt.as_deref().unwrap_or(DEFAULT_CAPTION_PROMPT);
    ...
    Ok(truncate_caption(&caption, options.max_caption_length))
}
...
/// Truncate caption at word boundary if it exceeds the limit.
fn truncate_caption(caption: &str, max_len: usize) -> String {
    if caption.chars().count() <= max_len {
...
impl CaptionError {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            Self::EmptyCaption => "empty_caption",
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn truncate_caption_preserves_utf8_boundaries() {
        assert_eq!(truncate_caption("hello world", 7), "hello");
        assert_eq!(truncate_caption("こんにちは 世界", 5), "こんにちは");
        assert_eq!(truncate_caption("emoji 😀 test", 8), "emoji 😀");
    }
}
