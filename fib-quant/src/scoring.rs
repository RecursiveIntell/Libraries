...
impl FibScorer {
    ...
    pub fn inner_product_estimate(&self, query: &[f32], code: &FibCodeV1) -> Result<f32> {
        ...
        let codewords_f64: Vec<f64> = codewords.iter().map(|&v| v as f64).collect();
        ...
        for (block_idx, stored_idx) in stored_indices.iter().enumerate() {
            ...
            let query_block_f64: Vec<f64> = query_block.iter().map(|&v| v as f64).collect();
            let query_idx = nearest_codeword(&query_block_f64, &codewords_f64, n, k);
    ...
    pub fn prepare_query(&self, query: &[f32]) -> Result<FibPreparedQuery> {
        ...
        let codewords_f64: Vec<f64> = codewords.iter().map(|&v| v as f64).collect();
        let n = self.quantizer.profile().codebook_size as usize;
        let query_indices: Vec<u32> = rotated_query_f32
            .chunks_exact(k)
            .map(|block| {
                let block_f64: Vec<f64> = block.iter().map(|&v| v as f64).collect();
                nearest_codeword(&block_f64, &codewords_f64, n, k) as u32
            })
            .collect();
...
/// Simple linear scan to find the nearest codeword for one block.
/// Used instead of the FFI `c_encode_vector_block` since this crate
/// forbids unsafe code.
fn nearest_codeword(block: &[f64], codewords: &[f64], n: usize, k: usize) -> usize {
    let mut best = 0usize;
    let mut best_dist = f64::MAX;
    for i in 0..n {
        let mut dist = 0.0f64;
        for d in 0..k {
            let diff = block[d] - codewords[i * k + d];
            dist += diff * diff;
        }
        if dist < best_dist {
            best_dist = dist;
