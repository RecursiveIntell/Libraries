...
use crate::types::{EtaConfidence, EtaEstimate, SizeBucket};
...
impl EtaTracker {
    ...
    pub fn record(
        ...
        match self.data.lock() {
            ...
            Err(e) => {
                tracing::warn!("ETA stats mutex poisoned, recovering: {}", e);
            }
        }
    }
    ...
    pub fn estimate(
        ...
        let remaining_ms = self.estimate_remaining(resource_key, operation, remaining_buckets)?;
        let mut total_avg_ms: u64 = 0;
        let mut matched_items: usize = 0;
        let mut sample_count: u64 = 0;

        for &bucket in remaining_buckets {
            if let Some(avg_ms) = self.estimate_one(resource_key, operation, bucket) {
                total_avg_ms += avg_ms;
                matched_items += 1;
                sample_count += self.sample_count(resource_key, operation, bucket).max(1);
            }
        }

        let avg_item_ms = if matched_items > 0 {
            total_avg_ms / matched_items as u64
        } else {
            0
        };
        let confidence = match sample_count {
            0..=2 => EtaConfidence::Low,
            3..=9 => EtaConfidence::Medium,
            _ => EtaConfidence::High,
        };

        Some(EtaEstimate {
            remaining_ms,
            items_remaining: remaining_buckets.len(),
            avg_item_ms,
            confidence,
            sample_count,
        })
