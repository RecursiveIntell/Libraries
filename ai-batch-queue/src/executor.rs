    ...
    H: BatchItemHandler<D>,
{
    ...
    for item in &job.items {
            ...
            && handler.should_skip(&item.data, &job.operation)
        {
            ...
            let _ = app_handle.emit(
                ...
                BatchItemProgressEvent {
                    ...
                    duration_ms: None,
                    eta_remaining_ms: eta.as_ref().map(|estimate| estimate.remaining_ms),
                    eta,
                },
            );
            continue;
        ...
        // Stamp a fresh TrialId for this concrete execution attempt.
        // This happens regardless of whether the item has an AttemptId —
        // the TrialId tracks "this specific run" for diagnostics.
        queue.stamp_trial_id(&job_id, &item.id);
    ...
    match queue.mark_completed(&job_id) {
        ...
        Ok(None) => {}
        Err(e) => tracing::error!(job_id = %job_id, error = %e, "Failed to mark job completed"),
    }
}
