...
- compressed-scorer real-corpus gate: `run_compressed_scorer_real_corpus_eval` evaluates true compressed-domain PerDim candidate scoring with exact f32 rerank, zero document decodes during candidate scoring, and `compressed-scorer-real-corpus-eval-v1` receipts. PerDim now uses query-prepared lookup-table contribution scoring. Stored Scifact/all-minilm result: R@10 0.7767, top-K overlap 0.9891, exact-rerank recovery@1 0.8767, compression 3.9588x, passed.
- compressed attention fixture gate: `run_compressed_attention_eval` compares exact top-k attention aggregation against `compressed-scorer::AttentionCache`, records top-k value decodes, and emits `compressed-attention-eval-v1` receipts.
