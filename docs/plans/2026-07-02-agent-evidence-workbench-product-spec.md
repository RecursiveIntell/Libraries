...
## 2026-07-16 research integration addendum

Source: `/home/sikmindz/Downloads/deep-research-report (2).md`  
SHA-256: `31f5a05e2e1c438b00bed16c3e9c6c153b635b97bd8408b9c7bebd25d4368927`

The later external research independently reinforces this specification's central decision: do not build another memory server or generic agent framework. Productize the existing trust boundary so an operator can see why an agent believed a claim, which tool executed, what evidence existed, and whether the resulting assertion or action was authorized.

### Portfolio relationship

The Workbench is the operator-facing product expression of a broader witnessed coding-agent substrate. Two adjacent compositions should reuse the same owner-qualified contracts rather than create parallel artifact families:

1. **Witnessed coding-agent substrate — primary product lane.** Join retrieval evidence, tool events, claim support, assertion/action authority, context-loss receipts, and deterministic replay.
2. **Release truth gate — first internal proving lane.** Block release and benchmark claims when source/package parity, command scope, artifact identity, or evidence is missing or stale.
3. **Operator memory OS — later public expansion.** Expose ranking explanations, contradiction/supersession state, and bitemporal views over promoted claims and evidence anchors.

These are product recommendations, not evidence that the products are complete.

### Contract correction and priority

The research proposes three thin cross-stack artifacts: a canonical envelope, a run receipt, and a claim-support receipt. Implementation must preserve existing owner authority instead of introducing a generic duplicate envelope. The current contract-freeze direction is therefore:

- retain the canonical owner-native export/import envelope path;
- evolve the existing AiDENs run bundle into the witnessed-run receipt surface;
- add a typed claim-support receipt owned by the claim/evidence layer;
- keep recorder output distinct from independent verification and action-authority decisions;
- keep lossy legacy conversion migration-only and ineligible for release admission.

### Research lanes

Production-near candidates are ColBERTv2/late interaction, Matryoshka-compatible embeddings, compiled parallel tool graphs, and DSSE/SCITT/Sigstore-style artifact attestations. GraphRAG-style community projections are relevant only as derived, freshness-receipted views. KV pruning, low-bit codec extensions, and trusted benchmark execution remain research lanes and must not be promoted without workload-bound replayable evidence.

### Claim boundary

The report's strongest support comes from public split repositories, published crates, and reproducibility artifacts. It could not independently inspect the private/inaccessible `Libraries` monorepo, exact workflow state, or PR metadata. Its architecture and product conclusions are useful research evidence, but not proof of current implementation, production maturity, or release readiness.
