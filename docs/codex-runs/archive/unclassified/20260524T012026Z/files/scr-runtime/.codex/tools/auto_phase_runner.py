#!/usr/bin/env python3
"""Automated phase runner for SCR Codex completion."""
    ...
    for phase in phases:
        ...
        if args.execute:
            ...
            if result.returncode != 0:
                ...
                receipt["errors"].append(f"phase {phase['id']} codex exec failed: returncode={result.returncode}")
        ...
        if args.run_required:
            ...
            if failed_commands:
                phase_record["status"] = "failed"
                receipt["errors"].append(f"phase {phase['id']} had {failed_commands} failed required command(s)")
                receipt["status"] = "failed"
