...
|---|---|---|---|---|---|---|
| Phase 0 — release truth and front door | P0 | Packaging truth | PACK-001 | closed | Ship blocker | Restore the numbered pack by adding `04_MASTER_ISSUE_MATRIX.csv` |
| Phase 0 — release truth and front door | P0 | Archive truth | PACK-002 | closed | Ship blocker | Fix the root archive manifest file count mismatch |
| Phase 0 — release truth and front door | P0 | Status truth | TRUTH-001 | closed | Truth blocker | Rewrite status surfaces from current script truth, not historical green claims |
| Phase 0 — release truth and front door | P0 | Gate convergence | GATE-001 | closed | Truth blocker | Make `make gate`, the proof ledger, and the receipt describe the same release lane |
| Phase 0 — release truth and front door | P0 | Safety gate hygiene | SAFE-001 | closed | Release-truth blocker | Fix the supported-lane panic audit so it measures production code instead of inline test modules |
| Phase 1 — CI and production closure | P1 | CI surface | CI-001 | closed | CI blocker | Add a real `.github/workflows/ci.yml` for the claimed release lane |
| Phase 1 — CI and production closure | P1 | Shipped script truth | V25-001 | closed | Pack blocker | Restore or retire the broken v25 repo-truth and production-pack surfaces |
| Phase 1 — CI and production closure | P1 | Production closure | V25-002 | closed | Conformance blocker | Complete the v25 production-closure marker set across effect/policy/control surfaces |
| Phase 1 — CI and production closure | P1 | Shared primitives | TYPE-001 | closed | Drift blocker | Centralize `V25ConstitutionCitation` and widen drift checking to the crates that currently duplicate it |
| Phase 2 — credibility and maintainability | P2 | Naming credibility | NAME-001 | closed | Credibility blocker | Stop overcalling thin governance crates “runtime” crates unless they earn it |
| Phase 2 — credibility and maintainability | P2 | Docs scope | DOC-001 | closed | Docs blocker | Expand the doc-truth story beyond the curated core-crate list |
| Phase 2 — credibility and maintainability | P2 | Module shape | MOD-001 | closed | Maintainability blocker | Break up the oversized modules that still dominate review difficulty |
| Phase 2 — credibility and maintainability | P2 | LLM integration | LLM-001 | closed | Clarity blocker | Either implement real `llm-refinement` or remove the feature flag and config path |
| Phase 2 — credibility and maintainability | P2 | Bootstrap extraction | EXTRACT-001 | closed | Correctness blocker | Replace or sharply bound the line-based Rust symbol extractor |
| Phase 2 — credibility and maintainability | P2 | Root pack hygiene | ROOT-001 | closed | Package polish | Collapse the duplicated root pack surfaces into one active authority lane |

## Status update

As of 2026-03-23, the Phase 0 release-truth lane and the Phase 1 CI/v25/type-closure lane are locally closed with proof artifacts.
`NAME-001`, `DOC-001`, `MOD-001`, `LLM-001`, and `EXTRACT-001` are also locally closed by current repo truth:
- `python3 scripts/check_public_api_docs.py` now distinguishes the 17-crate doc-certified lane from demoted compatibility-name crates and requires honest shell-crate surfaces.
- `bash scripts/check_hotspot_budgets.sh` now enforces the documented large-module exception set instead of implying those files are simply absent from review risk.
- `rg -n "use_llm_refinement|llm_model|llm-refinement" forge-pilot -g '!target/**'` returns no matches.
- `cargo test -p forge-pilot --test bootstrap_rust_extractor_degradation_tests -- --nocapture` passes and proves explicit extractor degradation on cfg/attribute/generic/multiline Rust surfaces.
- `python3 scripts/check_root_archive_manifest.py` now passes with a slimmer active-root manifest and a dedicated `root_pack_duplicates_20260323` archive bucket, while `bash scripts/check_pack_truth.sh` still passes for the retained numbered hostile pack.
