...
fn profile_p4_fixture_bundles_parse_into_owned_types() {
    for name in [
        ...
        if let Some(value) = artifacts.get("RegulatoryRegimeProfileV1") {
            let _: RegulatoryRegimeProfileV1 =
                serde_json::from_value(value.clone()).expect("RegulatoryRegimeProfileV1");
        }
        if let Some(value) = artifacts.get("RequirementControlMapV1") {
            let _: RequirementControlMapV1 =
                serde_json::from_value(value.clone()).expect("RequirementControlMapV1");
        }
        if let Some(value) = artifacts.get("EvidenceCollectionPlanV1") {
            let _: EvidenceCollectionPlanV1 =
                serde_json::from_value(value.clone()).expect("EvidenceCollectionPlanV1");
        }
        if let Some(value) = artifacts.get("RecertificationScheduleV1") {
            let _: RecertificationScheduleV1 =
                serde_json::from_value(value.clone()).expect("RecertificationScheduleV1");
        }
    }
}
