...
fn hazard_library_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/hazard-library-v1.example.json")
        .expect("read example");
...
fn hazard_scenario_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/hazard-scenario-v1.example.json")
        .expect("read example");
...
fn monitor_catalog_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/monitor-catalog-v1.example.json")
        .expect("read example");
...
fn mitigation_playbook_v1_roundtrips() {
    let body = std::fs::read_to_string("../examples/mitigation-playbook-v1.example.json")
        .expect("read example");
